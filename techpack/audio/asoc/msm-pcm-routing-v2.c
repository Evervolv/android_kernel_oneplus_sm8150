								...
								int cal_index)
{
	...
	list_for_each_safe(ptr, next,
		...
		if (((struct audio_cal_info_adm_top *)cal_block
			->cal_info)->path == path) {
			if (((struct audio_cal_info_adm_top *)cal_block->cal_info)->acdb_id == 102) {
				return NULL;
			}
			return cal_block;
		}
				...
				int topology, int copp_idx)
{
	if (loopback_map.active == true &&
		...
		pr_info("%s: deinit loopback map pp\n", __func__);
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		if (!is_be_dai_extproc(i) &&
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions[0]))) {
			...
			msm_pcm_cfg_loopback_map_pp(port_id, perf_mode,
					path_type, topology, copp_idx, acdb_dev_id);
		}
	}
	if (num_copps) {
		payload.num_copps = num_copps;
